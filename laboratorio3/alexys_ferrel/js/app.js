...
const PATRON_PALABRA = /[\p{L}\p{N}]+(?:[\s'\-][\p{L}\p{N}]+)*/gu;
...
function capitalizarNombre(texto) {
  return colapsarEspacios(texto)
    ...
    .replace(/(?:^|[\s'\-])\p{L}/gu, coincidencia => coincidencia.toLocaleUpperCase("es-PE"));
}
...
function extraerEtiquetas(texto) {
  ...
  return [...new Set(encontradas.map(etiqueta => etiqueta.toLocaleLowerCase("es-PE")))];
}
