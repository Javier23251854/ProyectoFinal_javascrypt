...
function validarDatos(datos) {
  const errores = [];
  if (normalizarTexto(datos.nombre).length < 3) errores.push("El nombre debe tener al menos 3 caracteres.");
  if (normalizarTexto(datos.categoria).length < 2) errores.push("La categoría debe tener al menos 2 caracteres.");
  if (!Number.isFinite(datos.precio) || datos.precio <= 0) errores.push("El precio debe ser mayor que cero.");
  if (!Number.isInteger(datos.stock) || datos.stock < 0) errores.push("El stock debe ser un entero >= 0.");
  return errores;
}
...
function validarColeccionImportada(valor) {
  if (!Array.isArray(valor)) throw new TypeError("El JSON debe contener un arreglo de productos.");
  ...
  if (new Set(ids).size !== ids.length) errores.push("Los identificadores no pueden repetirse.");
  if (productos.some(p => p.id.trim() === "")) errores.push("Todos los productos deben incluir un ID.");
  if (errores.length > 0) throw new TypeError(errores.join(" "));
  return productos;
}
...
function actualizarResumen() {
  ...
  alerta.textContent = hayAgotados ? "Atención: existe al menos un producto agotado." : "No existen productos agotados.";
}

function renderizar() {
  const filtrados = filtrarProductos(inventario, { texto: busqueda.value, categoria: filtroCategoria.value });
...
function manejarEliminacion(evento) {
  ...
  if (!producto) return;
  inventario = inventario.filter(e => e.id !== producto.id);
  sincronizarEstructuras();
