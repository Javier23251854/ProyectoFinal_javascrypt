...
const DATOS_INICIALES = [
 { id: "PRD-1001", nombre: "Monitor 24 pulgadas", categoria: "Pantallas", precio: 899.9, stock: 10 },
 { id: "PRD-1002", nombre: "Teclado mecánico", categoria: "Periféricos", precio: 219.5, stock: 2 },
 { id: "PRD-1003", nombre: "Mouse ergonómico", categoria: "Periféricos", precio: 129.9, stock: 3 },
...
// ==========================================
// MEJORA 1: Editar stock sin mutar (Inmutabilidad)
// ==========================================
/**
 * Actualiza el stock de un producto devolviendo un nuevo arreglo sin mutar los datos anteriores.
 * Utiliza Array.prototype.map para crear nuevas instancias de Producto.
 */
function actualizarStockProducto(productos, id, nuevoStock) {
...
function manejarAjusteStock(evento) {
  const boton = evento.target.closest("button[data-accion]");
  if (!boton) return;

  const id = boton.dataset.id;
  const accion = boton.dataset.accion;
  const producto = indicePorId.get(id);
  ...
  if (nuevoStock < 0) {
    mostrarMensaje("El stock no puede ser menor a 0.");
    return;
  }

  // Actualización inmutable
  inventario = actualizarStockProducto(inventario, id, nuevoStock);
  sincronizarEstructuras();
  mostrarMensaje(`Stock de "${producto.nombre}" actualizado a ${nuevoStock}.`, "exito");
}


// ==========================================
// MEJORA 2: Mostrar el producto de mayor valor
// ==========================================
/**
 * Encuentra el producto con mayor valor total en inventario (precio * stock) usando reduce.
 */
function obtenerProductoMayorValor(productos) {
  if (productos.length === 0) return null;
  return productos.reduce((max, p) => 
    p.valorInventario > max.valorInventario ? p : max, 
    productos[0]
  );
}

// Modificación de la función actualizarResumen para renderizar la Mejora 2
function actualizarResumen() {
  const resumen = calcularResumen(inventario);
  document.querySelector("#totalProductos").textContent = resumen.productos;
  document.querySelector("#totalUnidades").textContent = resumen.unidades;
  ...
  // Renderizar producto con mayor valor
  const productoTop = obtenerProductoMayorValor(inventario);
  const elementoTop = document.querySelector("#productoMayorValor");
  if (elementoTop) {
    elementoTop.textContent = productoTop 
      ? `${productoTop.nombre} (${formateadorMoneda.format(productoTop.valorInventario)})`
      : "N/A";
  }
...
// ==========================================
// ACTUALIZACIÓN DE INTERFAZ & EVENTOS
// ==========================================

// Actualizamos crearFila para incluir los botones de ajuste de stock [+1] [-1]
function crearFila(producto) {
  const fila = document.createElement("tr");
  const claseStock = producto.tieneStockBajo ? "numero stock-bajo" : "numero";
  ...
  btnSumar.dataset.accion = "incrementar";

  const btnRestar = document.createElement("button");
  btnRestar.type = "button";
  btnRestar.textContent = "-";
  btnRestar.dataset.id = producto.id;
  btnRestar.dataset.accion = "decrementar";
  ...
  contenedorAcciones.append(btnSumar, btnRestar, btnEliminar);

  const celdaAccion = document.createElement("td");
  celdaAccion.append(contenedorAcciones);
  fila.append(celdaAccion);

  return fila;
}

// Asignar los eventos de ajuste de stock delegando sobre la tabla
cuerpoInventario.addEventListener("click", manejarAjusteStock);